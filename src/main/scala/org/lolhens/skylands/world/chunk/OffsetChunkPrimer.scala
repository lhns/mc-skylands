...
class OffsetChunkPrimer(primer: ChunkPrimer, offset: Int) extends ChunkPrimer {
  override def setBlockState(x: Int, y: Int, z: Int, state: IBlockState): Unit = primer.setBlockState(x, y + offset, z, state)

  override def getBlockState(x: Int, y: Int, z: Int): IBlockState = primer.getBlockState(x, y + offset, z)
